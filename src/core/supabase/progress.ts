import { getBiggestT, getCorrectsT, getQuestionT, resetProgressT, SupabaseResponse, updateProgressContext, UpdateResultParams } from "../types";
import { supabase } from "./index";

export async function updateProgress({ user_id, isTrue, language }: updateProgressContext): Promise<void> {
    try {
        const response = await supabase.from("progress").select("*").eq("user_id", user_id);
        ...
        if (progressData && progressData.length === 0) {
            const { error: insertError } = await supabase.from("progress").insert([{ user_id: user_id }]);
            ...
            const { error: updateError } = await supabase
                .from("progress")
                .update({
                    [language]: isTrue ? progressData[0][language] + 1 : progressData[0][language],
                })
                .eq("user_id", user_id);
...
export async function updateResult({ user_id, language, value }: UpdateResultParams): Promise<void> {
    try {
        const response: SupabaseResponse<null> = await supabase.from("result").upsert({ user_id, [language]: value }, { onConflict: "user_id" });
...
export async function getLastCallback(language: string): Promise<number | null> {
    try {
        const { data, error } = await supabase.from(language).select("id").order("id", { ascending: false }).limit(1).single();
...
export async function getBiggest({ lesson_number, language }: getBiggestT): Promise<string | null> {
    try {
...
export async function getQuestion({ ctx, language }: getQuestionT): Promise<any[]> {
    try {
        console.log(ctx);
...
export async function resetProgress({ username, language }: resetProgressT): Promise<any> {
    try {
        ...
        const { data: progressData, error: progressError } = await supabase.from("progress").select("user_id").eq("user_id", userId);

        if (progressError) throw new Error(progressError.message);

        if (progressData && progressData.length === 0) {
            // Если записи нет, создаем новую
            const { error: insertError } = await supabase.from("progress").insert([{ user_id: userId }]);

            if (insertError) throw new Error(insertError.message);
        } else {
            const { data: dataProgress, error: errorProgress } = await supabase.from("progress").select("*").eq("user_id", userId).single();
...
export async function getCorrects({ user_id, language }: getCorrectsT): Promise<number> {
    try {
        if (user_id !== undefined) {
            const { data: dataCorrects, error: errorCorrects } = await supabase.from("progress").select("*").eq("user_id", user_id).single();
